...
public class WithdrawlTest extends BaseTest {
    ...
    public void withdrawalTest() {
        ...
        //Make withdrawl (Negative)
        customerPage.withdrawlButton.click();
        wait.until(ExpectedConditions.visibilityOf(customerPage.makeWithdrawlButton));
        customerPage.amountField.sendKeys("11");
        customerPage.makeWithdrawlButton.click();
        Assert.assertTrue(isElementPresent(By.xpath("//span[contains(text(),'Transaction Failed. You can not withdraw amount mo')]")));

        //Make withdrawal(Positive)
        customerPage.amountField.sendKeys("5");
        customerPage.makeWithdrawlButton.click();

        //Check balance
        accountInfo = driver.findElement(By.xpath("//div[contains(text(),'Account')]")).getText().split(",");
        balance = accountInfo[1];
        balanceArray = balance.split(" : ");
        userBalance = balanceArray[1];
        trimAccountNumber = userBalance.trim();
        int newUserBalance = Integer.parseInt(trimAccountNumber);
        Assert.assertEquals(newUserBalance, intUserBalance-5);

        //Check Transactions
        driver.navigate().refresh();
        customerPage.transactionsButton.click();
        wait.until(ExpectedConditions.visibilityOf(customerPage.withdrawAmountCell));
        Assert.assertEquals(customerPage.withdrawAmountCell.getText(),"5");
        Assert.assertEquals(customerPage.withdrawTransactionTypeCell.getText(),"Debit");
    }
}
